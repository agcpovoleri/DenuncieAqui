...
//					'Timestamp: '         + position.timestamp                + '\n');
		...
		createThumbnail: function(){
			...
			var $img = this.$el.find('div#imagensUpload img.img-thumbnail').last();
...
//			var $img = this.$el.find('div#imagensUpload img.img-thumbnail').last();
//
//			var srcdata = "data:image/jpeg;base64," + imageURI;  // <-- success alert
...
//
//			imagesToUpload.push({'imageURI': imageURI, "options": options});
//
//
//		},
//
...
//				});  
			...
			if (imagesToUpload != undefined && imagesToUpload.length > 0){
				for (var i=0; i < this.imagesToUpload.length; i++){
					var imgUp = this.imagesToUpload[i];
					var imageURI = imgUp.imageURI;
					var options = imgUp.options;

					alert(JSON.stringify(options));
					ft = new FileTransfer();
					ft.upload(imageURI, encodeURI(AppConfig.globalUrl + "/denuncia/image"), success, fail, options);
				}
			}
			return true;
		},
...
//							Camera.PictureSourceType.PHOTOLIBRARY
		...
		addFileToList: function(imageURI){
			var fail, ft, options, params, win;
			// callback for when the photo has been successfully uploaded:
			success = function(response) {
				alert("Your photo has been uploaded!");
